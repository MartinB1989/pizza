...
class MigrationManager {
    ...
    public function __construct() {
        ...
        $this->migrationsDir = __DIR__ . '/files';
    ...
    public function rollbackMigrations($steps = 1) {
        ...
        // Limitar el número de migraciones a revertir
        if ($steps > 0) {
        ...
        foreach ($migrations as $migrationName) {
            ...
            if (!file_exists($migrationFile)) {
                $results['errors'][] = [
                    'migration' => $migrationName,
                    'error' => "Archivo de migración no encontrado: $migrationName"
                ];
                continue;
            }
            ...
            try {
                ...
                // Verificar si hay instrucciones para revertir
                if ($sql === null) {
                    $results['errors'][] = [
                        'migration' => $migrationName,
     ...
     * 
     * @return array Historial de migraciones
     */
    public function getMigrationHistory() {
        $query = "SELECT migration, executed_at FROM migrations ORDER BY executed_at ASC, id ASC";
